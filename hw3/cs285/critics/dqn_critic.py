...
class DQNCritic(BaseCritic):
    ...
    def _build(self, q_func):
        ...
        if self.double_q:
            ...
            q_critc_tp1 = q_func(self.obs_tp1_ph, self.ac_dim, scope = 'q_func', reuse = True)
            amax = tf.argmax(q_critc_tp1, axis=1)
            q_tp1 = tf.reduce_max(q_tp1_values*tf.one_hot(amax, self.ac_dim), axis=1) 
        else:
