...
class ACAgent(BaseAgent):
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        V_s_1 = V_s_1*(1-terminal_n)

        adv_n = re_n+V_s_1*self.gamma-V_s
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        for _ in range(self.agent_params['num_actor_updates_per_agent_update']):
            agent_loss = self.actor.update(ob_no, ac_na, advantage)

        loss = OrderedDict()
        loss['Critic_Loss'] = critic_loss  # put final critic loss here
        loss['Actor_Loss'] = agent_loss  # put final actor loss here
        return loss
